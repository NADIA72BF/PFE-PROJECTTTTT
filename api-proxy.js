...
// Zoho Analytics public dashboard URL (used by chatbot SHOW_CHART action)
const ANALYTICS_DASHBOARD_URL = process.env.ZOHO_ANALYTICS_DASHBOARD_URL
  || 'https://analytics.zoho.com/open-view/3208169000000460126/3b8784c3ab48dd254c9d21d736b157e735b345dbcfe38f5b3843593eb1eb63c9';
...
const REPORT_CACHE_TTL       = 30000;   // 30 seconds
const OWNER_PROPS_CACHE_TTL  = 120000;  // 2 minutes
...
const ownerPropsCache         = new Map();
const inFlightRequests        = new Map();

// Pending signups — email not yet verified, record not yet in Zoho
// Key: token (hex), Value: { email, first_name, last_name, phone_number, password, role, createdAt }
const pendingSignups = new Map();
const PENDING_SIGNUP_TTL = 24 * 60 * 60 * 1000; // 24 hours

setInterval(() => {
  const now = Date.now();
  for (const [token, data] of pendingSignups.entries()) {
    if (now - data.createdAt > PENDING_SIGNUP_TTL) pendingSignups.delete(token);
  }
}, 60 * 60 * 1000);
...
// Deduplicates simultaneous identical GET requests to Zoho
async function fetchZohoJsonDeduped(url, options) {
  const key = `${options?.method || 'GET'}:${url}`;
  if (inFlightRequests.has(key)) return inFlightRequests.get(key);
  const promise = fetchZohoJson(url, options).finally(() => inFlightRequests.delete(key));
...
async function fetchUserByEmail(email) {
  ...
  if (cached && Date.now() - cached.at < USERS_CACHE_TTL) return cached.data;

  const criteria = encodeURIComponent(`Email == "${email}"`);
  const { payload } = await fetchZohoJson(
    `${BASE_CREATOR}/report/All_Users?criteria=${criteria}`,
    { fallbackMessage: 'Erreur recherche utilisateur' }
  );
  const user = (Array.isArray(payload?.data) ? payload.data : [])[0] || null;
  if (user) usersCache.set(cacheKey, { at: Date.now(), data: user });
  return user;
}
...
async function fetchUserById(userId) {
  ...
  const { payload } = await fetchZohoJson(
    `${BASE_CREATOR}/report/All_Users/${userId}?field_config=all`,
    { fallbackMessage: 'Erreur récupération utilisateur' }
  );
  const user = payload?.data?.[0] || payload?.data || payload || null;
  if (user) usersCache.set(cacheKey, { at: Date.now(), data: user });
  return user;
}

function invalidateUserCache(email, userId) {
  if (email)  usersCache.delete(`user:${email}`);
  if (userId) usersCache.delete(`user:id:${userId}`);
}
...
// Fetch a Zoho report with multi-endpoint fallback (BASE_V21 → BASE_CREATOR → BASE_CREATORAPP).
async function fetchReportWithFallback(reportName, fallbackMessage) {
  const ts = Date.now();
  const candidates = [
    { url: `${BASE_V21}/report/${reportName}?field_config=all&max_records=200&_ts=${ts}`, authType: 'oauthtoken', retries: 2 },
    { url: `${BASE_CREATOR}/report/${reportName}?field_config=all&max_records=200&_ts=${ts}`, authType: 'bearer', retries: 2 },
    { url: `${BASE_CREATORAPP}/report/${reportName}?field_config=all&max_records=200&_ts=${ts}`, authType: 'bearer', retries: 2 },
    { url: `${BASE_CREATOR}/report/${reportName}`, authType: 'bearer', retries: 3 },
  ];
  let lastError;
  for (const cand of candidates) {
    try {
      const { payload } = await fetchZohoJson(cand.url, { authType: cand.authType, retries: cand.retries, fallbackMessage: fallbackMessage || `Erreur ${reportName}` });
      return payload;
    } catch (err) {
      lastError = err;
  ...
  throw lastError || new Error(fallbackMessage || `Erreur ${reportName}`);
}
...
// Fetch all records of a Zoho report — tries BASE_V21 (oauthtoken) first, then BASE_CREATOR, then BASE_CREATORAPP.
async function fetchAllRecords(reportName, fallbackMessage) {
  const cacheKey = `report:${reportName}`;
  const cached = reportCache.get(cacheKey);
  if (cached && Date.now() - cached.at < REPORT_CACHE_TTL) return cached.data;
...
async function fetchOwnerPropertyIds(req) {
  ...
  const cacheKey = `owner:${userId}:${userId1}`;
  const cached = ownerPropsCache.get(cacheKey);
  if (cached && Date.now() - cached.at < OWNER_PROPS_CACHE_TTL) return cached.ids;
  ...
  ownerPropsCache.set(cacheKey, { at: Date.now(), ids });
  return ids;
}
...
app.post('/api/login', loginLimiter, async (req, res) => {
  try {
    ...
    try {
      const listUser = await fetchUserByEmail(normalizedEmail);
      if (!listUser) {
        return res.status(401).json({ error: 'Email ou mot de passe incorrect' });
      }

      const userId  = listUser.ID;
      ...
      req.session.userId = fullUser.ID || fullUser.id;
      return createUserSession(req, res, fullUser, { source: 'zoho' });
    } catch (err) {
...
app.post('/api/auth/forgot-password', forgotPasswordLimiter, async (req, res) => {
  try {
    ...
    await fetchZohoJson(
      `${BASE_CREATOR}/report/All_Users/${userId}`,
      {
        method: 'PATCH',
        body: { data: { Reset_Token: token, Reset_Token_Expiry: expiry } },
        fallbackMessage: 'Erreur mise à jour token'
      }
    );

    invalidateUserCache(normalizedEmail, userId);
...
app.post('/api/auth/verify-reset-token', async (req, res) => {
  try {
    ...
    const listUser = await fetchUserByEmail(normalizedEmail);
    if (!listUser) return res.status(404).json({ error: 'Compte introuvable' });

    const fullUser = await fetchUserById(listUser.ID);
    const storedToken  = fullUser?.Reset_Token        || '';
    const storedExpiry = fullUser?.Reset_Token_Expiry || '0';
...
app.post('/api/auth/reset-password', async (req, res) => {
  try {
    ...
    if (!email || !token || !newPassword) return res.status(400).json({ error: 'Tous les champs sont requis' });
    ...
    await fetchZohoJson(
      ...
      {
        method: 'PATCH',
        body: { data: { Password: newPassword, Reset_Token: '', Reset_Token_Expiry: '' } },
        fallbackMessage: 'Erreur mise à jour mot de passe'
      }
    );
...
app.get('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    if (!token || !email) return res.status(400).json({ error: 'Lien invalide' });

    const normalizedEmail = String(email).trim().toLowerCase();

    const pendingData = pendingSignups.get(token);

    if (!pendingData) {
      // Token not in memory — could be expired or already used; check if account exists
      const existing = await fetchUserByEmail(normalizedEmail).catch(() => null);
      if (existing) {
        return res.json({ success: true, alreadyVerified: true, message: 'Email déjà vérifié. Vous pouvez vous connecter.' });
      }
      return res.status(400).json({ error: 'Lien invalide ou expiré. Veuillez vous inscrire à nouveau.' });
    }

    if (pendingData.email !== normalizedEmail) {
      return res.status(400).json({ error: 'Lien invalide.' });
    }

    if (Date.now() - pendingData.createdAt > PENDING_SIGNUP_TTL) {
      pendingSignups.delete(token);
      return res.status(400).json({ error: 'Lien expiré. Veuillez vous inscrire à nouveau.' });
    }

    // Race-condition check — someone else may have registered same email
    try {
      const { payload } = await fetchZohoJson(
      ...
      if (!err.message.includes('404')) console.warn('[VerifyEmail] Email check failed:', err.message);
    }

    // Create the record in Zoho Creator now that email is confirmed
    let createData;
    try {
      const { payload } = await fetchZohoJson(`${BASE_CREATOR}/form/User`, {
        method: 'POST',
        authType: 'bearer',
        body: {
          data: {
            full_name: { first_name: pendingData.first_name, last_name: pendingData.last_name },
    ...
    if (!zohoRecordId || !/^\d+$/.test(String(zohoRecordId).trim())) {
      const msg = extractWorkflowAlertMessage(createData, 'Erreur création compte');
      return res.status(400).json({ error: msg });
    }

    pendingSignups.delete(token);
    console.log(`[VerifyEmail] Account created in Zoho for ${normalizedEmail}, ID: ${zohoRecordId}`);

    res.json({ success: true, message: 'Email vérifié ! Votre compte est actif. Vous pouvez vous connecter.' });
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    // Find existing pending signup by email
    let existingToken = null;
    let existingData  = null;
    for (const [t, d] of pendingSignups.entries()) {
      if (d.email === normalizedEmail) { existingToken = t; existingData = d; break; }
    }

    if (!existingToken) {
      return res.status(404).json({
        error: 'Aucune inscription en attente pour cet email. Veuillez vous inscrire à nouveau.'
      });
    }

    // Rotate token and reset expiry
    const newToken = crypto.randomBytes(32).toString('hex');
    pendingSignups.delete(existingToken);
    pendingSignups.set(newToken, { ...existingData, createdAt: Date.now() });

    const verifyLink = `${req.protocol}://${req.get('host')}/verify-email.html?token=${newToken}&email=${encodeURIComponent(normalizedEmail)}`;

    try {
      await sendVerificationEmailViaCreator(normalizedEmail, existingData.first_name, verifyLink);
    } catch (err) {
      console.warn('[Resend] Email send failed (new token stored):', err.message);
    }

    console.log(`[Resend] New verify link for ${normalizedEmail}: ${verifyLink}`);
    res.json({ success: true, message: 'Email de vérification renvoyé. Vérifiez votre boîte mail.' });
...
app.post('/api/logout', (req, res) => {
  ...
  const uid1 = req.session.userId1 || '';
  if (uid) ownerPropsCache.delete(`owner:${uid}:${uid1}`);
...
// ---------------------------------------------------------------------------
// EMAIL HELPER — Zoho Creator Custom API
// ---------------------------------------------------------------------------
...
async function sendVerificationEmailViaCreator(to_email, to_name, verify_link) {
    const form = new FormData();  // FormData already imported at top of file
    form.append('to_email', to_email);
    form.append('to_name', to_name);
    form.append('verify_link', verify_link);

    const response = await fetch(
        'https://www.zohoapis.com/creator/custom/2demonexflow/send_email_direct?publickey=DqrYjJBu80fMwTURzMt3Mtgz1',
        {
            method: 'POST',
            body: form,
            headers: {
    ...
    const text = await response.text();
    console.log(`[Email API] Status: ${response.status}, Response: ${text}`);
...
// ROUTES  SIGNUP
//
// POST /api/signup  (pending flow — two steps)
// Business logic handled by Creator workflows AFTER email verification:
//   - Add_User: validates password strength, email uniqueness, sets default role
//   - sync_user_to_crm: creates corresponding CRM contact
//
// Node.js handles:
//   1. POST /api/signup       → validate input, store in pendingSignups, send verification email via Creator Custom API
//   2. GET  /api/auth/verify-email → consume token, create record in Zoho Creator
//
app.post('/api/signup', async (req, res) => {
  try {
    ...
    // Check email not already in Zoho Creator
    try {
      ...
      if (!err.message.includes('404')) console.warn('[Signup] Email check failed:', err.message);
    }

    // Check not already pending
    for (const [, data] of pendingSignups.entries()) {
      if (data.email === normalizedEmail) {
        return res.status(400).json({
          error: 'Un email de vérification a déjà été envoyé à cette adresse. Vérifiez votre boîte mail.',
    ...
    const verifyLink = `${req.protocol}://${req.get('host')}/verify-email.html?token=${token}&email=${encodeURIComponent(normalizedEmail)}`;

    // Send verification email via Zoho Creator Custom API
    try {
      await sendVerificationEmailViaCreator(normalizedEmail, String(first_name).trim(), verifyLink);
    } catch (err) {
      console.warn('[Signup] Email send failed (token stored):', err.message);
    }

    console.log(`[Signup] Pending signup for ${normalizedEmail}, token: ${token.substring(0, 8)}...`);

    res.json({
      success: true,
      needsVerification: true,
      message: 'Un email de vérification a été envoyé. Vérifiez votre boîte mail pour activer votre compte.',
      email: normalizedEmail
    });
...
app.get('/api/properties/:id/contact', requireAuth, async (req, res) => {
  try {
    const property = await fetchZohoPropertyRecord(req.params.id);
    if (!property) return res.status(404).json({ error: 'Bien introuvable' });

    const ownerPhone = property?.User?.Phone_Number || '';
    const ownerName  = property?.User?.display_value || property?.User?.full_name || 'Propriétaire';

    res.json({
      success: true,
      phone: ownerPhone,
      name: ownerName,
      whatsappUrl: ownerPhone ? buildWhatsAppUrlServer(ownerPhone, property.title) : null
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

function buildWhatsAppUrlServer(phone, propertyTitle) {
  ...
  const message = encodeURIComponent(
    `Bonjour, je suis intéressé(e) par votre bien: "${propertyTitle || 'ce bien'}". ` +
...
// Business logic handled by Creator workflows:
//   - status&calcul: sets Status="En attente", Duration_Text, calculates amounts, Advance_Amount
//   - Check_Property_Availability: validates date conflicts, cancels on overlap
//   - status&calcul: validates property type restrictions
//
// Node.js only:
...
//   - notify_seller_on_purchase: emails the seller on new purchase request
//   - contrat_apres_acceptation: generates contract after advance payment confirmation
//
...
// ─────────────────────────────────────────────────────────────────────────────
// ROUTES — RECORD DETAIL (for deadline fetch)
// ─────────────────────────────────────────────────────────────────────────────

app.get('/api/reservations/detail/:id', requireAuth, async (req, res) => {
  try {
    const { payload } = await fetchZohoJson(
      `${BASE_CREATORAPP}/report/All_Reservations/${req.params.id}?field_config=all`,
      { fallbackMessage: 'Erreur récupération réservation' }
    );
...
app.get('/api/purchases/detail/:id', requireAuth, async (req, res) => {
  try {
    const { payload } = await fetchZohoJson(
      `${BASE_CREATORAPP}/report/All_Purchases/${req.params.id}?field_config=all`,
      { fallbackMessage: 'Erreur récupération achat' }
    );
    const data = payload?.data?.[0] || payload?.data || payload;
    res.json({ success: true, data });
  } catch (err) {
...
app.get('/api/payments/contract/:contractId', requireAuth, async (req, res) => {
  try {
    const id    = req.params.contractId;
    const altId = req.query.altId || '';

    // Attempt 1: criteria by display value (Zoho Creator lookup criteria match display_value)
    const criteria = encodeURIComponent(`Contract == "${id}"`);
    const { payload } = await fetchZohoJson(
      `${BASE_CREATORAPP}/report/All_Payments?criteria=${criteria}&field_config=all`,
      { fallbackMessage: 'Erreur récupération paiement' }
    );
    let data = payload.data || [];

    // Attempt 2: full scan matching any known identifier across multiple possible field names
    if (!data.length) {
      const ids = [id, altId].filter(Boolean);
      const all = await fetchAllRecords('All_Payments', 'Erreur paiements').catch(() => []);

      // Log structure of first record to diagnose field names
      if (all.length > 0) {
        console.log(`[payments/contract] report has ${all.length} records. Keys: ${Object.keys(all[0]).join(', ')}`);
        console.log(`[payments/contract] Contract field sample:`, JSON.stringify(all[0].Contract ?? all[0].Contrat ?? null));
      } else {
        console.log('[payments/contract] All_Payments report returned 0 records');
      }

      const contractFields = ['Contract', 'Contrat', 'Contract_ID', 'Contrat_ID', 'Lease', 'Bail'];
      data = all.filter(p => {
        return contractFields.some(fn => {
          const c = p[fn];
          if (!c) return false;
          const vals = (typeof c === 'string')
            ? [c]
            : [c.display_value, c.zc_display_value, c.ID, c.id, c.value].filter(Boolean).map(String);
          return ids.some(needle => vals.some(v => v.trim() === needle.trim()));
        });
      });
    }
...
app.patch('/api/payments/update/:paymentId', requireAuth, async (req, res) => {
  try {
    await fetchZohoJson(
      `${BASE_CREATORAPP}/report/All_Payments/${req.params.paymentId}`,
      {
        method: 'PATCH',
        body: { data: req.body },
        fallbackMessage: 'Erreur mise à jour paiement'
      }
    );
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
...
app.get('/api/payments/invoice-link/:paymentId', requireAuth, async (req, res) => {
  try {
    await ensureValidToken();
    const { payload } = await fetchZohoJson(
      `${BASE_CREATORAPP}/report/All_Payments/${req.params.paymentId}?field_config=all`,
      { fallbackMessage: 'Erreur récupération paiement' }
    );
    const payment = Array.isArray(payload?.data) ? payload.data[0] : payload?.data;
    const invoiceId = payment?.Zoho_Books_Invoice_ID;
    if (!invoiceId) return res.status(404).json({ error: 'Facture non disponible' });

    const booksResp = await fetch(
      `https://www.zohoapis.com/books/v3/invoices/${invoiceId}?organization_id=913662464`,
      { headers: { 'Authorization': `Zoho-oauthtoken ${ZOHO_ACCESS_TOKEN}` } }
    );
    const booksData = await booksResp.json();
    const invoice = booksData?.invoice;
    const paymentUrl = `https://books.zoho.com/portal/nexusgestionimmobiliere/invoices/${invoiceId}`;
    res.json({
      success: true,
      invoiceId,
...
app.post('/api/payments/advance', requireAuth, async (req, res) => {
  try {
    ...
    if (type === 'reservation') {
    // PATCH 1 — set advance payment status
    await fetchZohoJson(
        `${BASE_CREATORAPP}/report/All_Reservations/${id}`,
        {
            method: 'PATCH',
            body: {
                data: {
                    Advance_Payment_Status: 'Payé'
                }
            },
            fallbackMessage: 'Erreur paiement avance'
        }
    );
    
    // Small delay to let Creator process first update
    await new Promise(r => setTimeout(r, 1000));
    
    // PATCH 2 — set status to Confirmé → triggers generer_contrat_location
    await fetchZohoJson(
        `${BASE_CREATORAPP}/report/All_Reservations/${id}`,
        {
            method: 'PATCH',
            ...
            fallbackMessage: 'Erreur confirmation réservation'
        }
    );
} else if (type === 'purchase') {
    // PATCH 1 — advance payment
    await fetchZohoJson(
        `${BASE_CREATORAPP}/report/All_Purchases/${id}`,
        {
            method: 'PATCH',
            body: {
                data: {
    ...
    // PATCH 2 — triggers generer_contrat_achat
    await fetchZohoJson(
        `${BASE_CREATORAPP}/report/All_Purchases/${id}`,
        {
            method: 'PATCH',
            ...
            fallbackMessage: 'Erreur confirmation achat'
        }
    );
} else {
    ...
    console.log(`[Advance Payment] ${type} ${id} confirmed`);
    res.json({ success: true, message: 'Paiement confirmé!' });

  } catch (err) {
    console.error('[Advance Payment] Error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/agent/profile', requireAuth, async (req, res) => {
  try {
    ...
    const payload = await fetchReportWithFallback('All_Users', 'Erreur récupération profil agent');
...
app.get('/api/admin/properties', async (req, res) => {
  try {
    ...
    const payload = await fetchReportWithFallback(ADMIN_PROPERTIES_REPORT, 'Erreur recuperation des proprietes');
...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (!GROQ_API_KEY) return res.json({ reply: 'Le chatbot n\'est pas encore configuré. Veuillez contacter l\'administrateur.' });

    // Rôle utilisateur depuis la session
    const sessionRole = String(req.session?.userRole || '').trim();
    const isAdmin = ['Administrator', 'Admin', 'administrator', 'admin'].includes(sessionRole);
    const isAgent = sessionRole === 'Agent' || sessionRole === 'agent';

    // Propriétés disponibles (biens approuvés pour le contexte)
    let propertiesContext = 'Aucune propriété disponible pour le moment.';
    try {
      const port = Number(process.env.PORT || 3000);
      const localRes = await fetch(`http://localhost:${port}/api/properties`, { cache: 'no-store' });
      const localData = await localRes.json();
      const props = Array.isArray(localData?.data) ? localData.data : [];
      if (props.length > 0) {
        propertiesContext = props.slice(0, 20).map(p => {
          const title   = String(p.title || p.Title || p.Property_Title || 'Sans titre');
          const price   = String(p.Price1 || p.price || 'N/A');
          const loc     = typeof p.location === 'object'
            ? String(p.location?.display_value || p.location?.district_city || p.location?.address_line_1 || '')
            : String(p.location || 'N/A');
          const type    = String(p.type_field || p.type || 'N/A');
          const rooms   = String(p.Rooms1 || p.rooms || '?');
          const surface = String(p.Surface1 || p.surface || '?');
          const id      = String(p.ID1 || p.ID || '');
          return `• ${id ? '[ID:'+id+'] ' : ''}${title} | ${type} | ${price} DT | ${loc} | ${rooms} ch. | ${surface} m²`;
        }).join('\n');
      }
    } catch (e) { console.warn('[/chat] Properties fetch failed:', e.message); }
...
REGLE ABSOLUE : remplace toujours le chiffre réel dans la balise. Ne jamais écrire "ID" ou "NUMERO" dans la balise.
Exemples EXACTS à reproduire :
  User: "supprime le bien 350"   → Suppression en cours. [ACTION:DELETE_PROP:350]
  User: "supprime la propriété 42" → [ACTION:DELETE_PROP:42]
  User: "valide le bien 7"       → Validation en cours. [ACTION:APPROVE_PROP:7]
  User: "rejette la propriété 12" → Rejet en cours. [ACTION:REJECT_PROP:12]
  User: "supprime l'utilisateur 5" → [ACTION:DELETE_USER:5]
  User: "liste les utilisateurs"  → [ACTION:LIST_USERS]
  User: "propriétés en attente"      → [ACTION:LIST_PENDING]
  User: "montre les graphiques"      → [ACTION:SHOW_CHART:dashboard]
  User: "graphique des propriétés"   → [ACTION:SHOW_CHART:proprietes]
  User: "stats utilisateurs"         → [ACTION:SHOW_CHART:utilisateurs]
  User: "graphique réservations"     → [ACTION:SHOW_CHART:reservations]
  User: "graphique des contrats"     → [ACTION:SHOW_CHART:contrats]
  User: "graphique contrats par type"→ [ACTION:SHOW_CHART:contrats]
  User: "voir les statistiques"      → [ACTION:SHOW_CHART:dashboard]
RÈGLE ABSOLUE GRAPHIQUES : Pour toute demande de graphique, chart, visualisation, statistique, analytics → utilise UNIQUEMENT le format [ACTION:SHOW_CHART:type]. Ne génère JAMAIS un autre format de balise comme [VISUALIZE_...] ou autre.
Si l'utilisateur ne précise pas l'ID, demande-le. Réponds en français.`;
    } else if (isAgent) {
      roleInstructions = `
L'utilisateur connecté est AGENT.
REGLE ABSOLUE : remplace toujours le chiffre réel dans la balise. Ne jamais écrire "ID" ou "NUMERO" dans la balise.
Exemples EXACTS à reproduire :
  User: "valide le bien 7"        → Validation en cours. [ACTION:APPROVE_PROP:7]
  User: "rejette la propriété 12" → Rejet en cours. [ACTION:REJECT_PROP:12]
...
Si l'utilisateur ne précise pas l'ID, demande-le. Réponds en français.`;
      ...
      roleInstructions = `
RÈGLE ABSOLUE : Tu n'as accès à aucune information sur les utilisateurs, les comptes, les rôles ou les données administratives.
Si l'utilisateur demande la liste des utilisateurs, des comptes, des informations personnelles ou toute action d'administration : réponds UNIQUEMENT "Je ne suis pas autorisé à accéder à cette section." et rien d'autre.
Tu peux uniquement aider à trouver des biens immobiliers disponibles.`;
    }

    const systemPrompt = `Tu es Nexia, l'assistant IA de GI Immobilier (Tunisie).
Tu réponds en français. Sois concis, chaleureux et professionnel. Maximum 4 phrases sauf si nécessaire.
${roleInstructions}
...
${propertiesContext}`;
    ...
    if (!groqRes.ok) {
      ...
      return res.json({ reply: 'Désolé, le service IA est temporairement indisponible. Réessayez dans quelques instants.' });
    }

    const data = await groqRes.json();
    let reply = String(data?.choices?.[0]?.message?.content || 'Désolé, je n\'ai pas pu générer une réponse.').trim();

    // Exécution des actions détectées dans la réponse de Groq (admin/agent uniquement)
    const actionMatch = (isAdmin || isAgent)
      ? reply.match(/\[ACTION:([A-Z_]+)(?::([^\]]*))?\]/)
      : null;
    // Supprimer les balises action ET tout tag [MAJUSCULES_...] généré par le modèle
    reply = reply.replace(/\[[A-Z][A-Z0-9_:]*[^\]]*\]/g, '').trim();
    let chartData = null;
    if (actionMatch) {
      const action = actionMatch[1];
      const param  = (actionMatch[2] || '').trim();

      // Criteria-based DELETE (works with ID1 or system ID)
      async function zohoDeleteByCriteria(reportName, idInput) {
        await ensureValidToken();
        const criterias = [`(ID1 == "${idInput}")`, `(ID == "${idInput}")`];
        const bases = [
          { url: BASE_V21, auth: () => `Zoho-oauthtoken ${ZOHO_ACCESS_TOKEN}` },
          { url: BASE_CREATOR, auth: () => `Bearer ${ZOHO_ACCESS_TOKEN}` },
        ];
        for (const base of bases) {
          for (const crit of criterias) {
            try {
              const r = await fetch(`${base.url}/report/${reportName}?criteria=${encodeURIComponent(crit)}`, {
                method: 'DELETE',
                headers: { Authorization: base.auth() }
              });
              const text = await r.text().catch(() => '');
              const json = text ? JSON.parse(text) : {};
              if (r.status === 200 || r.status === 204 || json?.code === 3000) return true;
            } catch (_) {}
          }
        }
        return false;
      ...
      try {
        if (action === 'LIST_USERS' && isAdmin) {
          const { payload } = await fetchZohoJson(buildUsersReportUrl(), { retries: 3, fallbackMessage: 'Erreur liste utilisateurs' });
          const users = Array.isArray(payload?.data) ? payload.data : [];
          if (!users.length) {
            ...
            reply += '\n\nUtilisateurs (' + users.length + ') :\n' +
              users.slice(0, 15).map(u => {
                const name  = String(u.full_name || u.Full_Name || `${String(u.first_name||'')} ${String(u.last_name||'')}`.trim() || 'N/A');
                const email = String(u.Email || u.email || 'N/A');
                const id1   = String(u.ID1 || 'N/A');
                const role  = String(u.Role || u.User_Role || u.role || 'User');
                return `• [ID: ${id1}] ${name} | ${email} | ${role}`;
              }).join('\n');
            if (users.length > 15) reply += `\n... et ${users.length - 15} autres.`;
            reply += '\n\nPour supprimer, dites: "supprime l\'utilisateur [ID]"';
          }

        } else if (action === 'LIST_PENDING' && (isAdmin || isAgent)) {
          const payload = await fetchReportWithFallback('All_Properties', 'Erreur liste propriétés');
          const all = Array.isArray(payload?.data) ? payload.data : [];
          const pending = all.filter(p => isPropertyPending(p));
          if (!pending.length) {
            reply += '\n\nAucune propriété en attente de validation.';
          } else {
            reply += '\n\nPropriétés en attente (' + pending.length + ') :\n' +
              pending.slice(0, 10).map(p => {
                const title = String(p.title || p.Title || p.Property_Title || 'Sans titre');
                const id1   = String(p.ID1 || 'N/A');
                const price = String(p.Price1 || p.price || 'N/A');
                return `• [ID: ${id1}] ${title} | ${price} DT`;
              }).join('\n');
            reply += '\n\nPour valider: "valide la propriété [ID]" | Pour rejeter: "rejette la propriété [ID]"';
          }

        } else if (action === 'APPROVE_PROP' && param && (isAdmin || isAgent)) {
          const ok = await updatePropertyValidationStatusDirect(param, APPROVED_STATUS_VALUE);
          clearPropertiesResponseCache();
          reply += ok
            ? `\n\nPropriété [${param}] approuvée avec succès. Elle est maintenant visible sur le site.`
            : `\n\nImpossible d'approuver la propriété ${param}. Vérifiez l'ID dans la liste.`;

        } else if (action === 'REJECT_PROP' && param && (isAdmin || isAgent)) {
          const ok = await updatePropertyValidationStatusDirect(param, REJECTED_STATUS_VALUE);
          clearPropertiesResponseCache();
          reply += ok
            ? `\n\nPropriété [${param}] rejetée.`
            : `\n\nImpossible de rejeter la propriété ${param}. Vérifiez l'ID dans la liste.`;

        } else if (action === 'DELETE_PROP' && param && isAdmin) {
          const deleted = await zohoDeleteByCriteria(ADMIN_PROPERTIES_REPORT, param);
          clearPropertiesResponseCache();
          reply += deleted
            ? `\n\nPropriété [${param}] supprimée définitivement de Zoho Creator et du site.`
            : `\n\nImpossible de supprimer la propriété ${param}. Vérifiez l'ID dans la liste.`;

        } else if (action === 'DELETE_USER' && param && isAdmin) {
          const deleted = await zohoDeleteByCriteria('All_Users', param);
          reply += deleted
            ? `\n\nUtilisateur [${param}] supprimé de Zoho Creator.`
            : `\n\nImpossible de supprimer l'utilisateur ${param}. Vérifiez l'ID dans la liste.`;

        } else if (action === 'SHOW_CHART' && isAdmin) {
          const CHART_TITLES = {
            proprietes:   'Propriétés – Répartition & statuts',
            utilisateurs: 'Utilisateurs inscrits',
            reservations: 'Réservations & Achats',
            contrats:     'Contrats',
            dashboard:    'Tableau de bord analytique complet',
          };
          const title = CHART_TITLES[param] || CHART_TITLES['dashboard'];
          chartData = { url: ANALYTICS_DASHBOARD_URL, title };
          if (!reply) reply = `Voici le graphique **${title}** :`;
        }

      } catch (actionErr) {
        console.error('[/chat] Action error:', actionErr.message);
        reply += '\n\nErreur lors de l\'exécution de l\'action: ' + actionErr.message;
      }
    }

    const responsePayload = { reply: reply.trim() };
    if (chartData) responsePayload.chart = chartData;
    res.json(responsePayload);
  } catch (err) {
    console.error('[/chat] Error:', err.message);
    res.json({ reply: 'Une erreur est survenue. Réessayez plus tard.' });
  }
});
...
async function validateStartupToken() {
  try {
    await refreshAccessToken();
    console.log('OAuth token validated on startup');
  } catch (err) {
    console.error('OAuth token invalid on startup:', err.message);
    console.error('Please update ZOHO_REFRESH_TOKEN in .env');
  }
}
