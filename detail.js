...
async function loadPropertyDetails() {
  ...
  try {
    ...
    // Fetch owner contact details from user lookup
    await loadOwnerContact(property);
  ...
  reservationForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        console.log('Reservation created:', JSON.stringify(data));

        const reservationId = data.data?.data?.[0]?.ID ||
                              data.data?.data?.ID       ||
                              data.data?.ID             ||
                              data.reservationId;

        console.log('Reservation ID extracted:', reservationId);

        const start   = new Date(startDate);
        const end     = new Date(endDate);
        const days    = Math.max(1, Math.ceil((end - start) / (1000 * 60 * 60 * 24)) + 1);
        const price   = parseFloat(property.Price1) || 0;
        const totalAmount = price * days;
        const advance = totalAmount ? Math.ceil(totalAmount * 0.05) : 0;

        messageDiv.className = 'success-alert';
        messageDiv.innerHTML = '✓ Réservation créée ! Redirection vers le paiement…';
        messageDiv.style.display = 'block';

        if (!reservationId) {
          console.error('Could not extract reservation ID from:', JSON.stringify(data));
          messageDiv.className = 'error-alert';
          messageDiv.innerHTML = '⚠️ Réservation créée mais redirection impossible. Vérifiez vos réservations.';
          messageDiv.style.display = 'block';
        ...
        const paymentUrl = `advance-payment.html?type=reservation` +
          `&id=${encodeURIComponent(reservationId)}` +
          `&amount=${encodeURIComponent(advance)}` +
          `&property=${encodeURIComponent(property.title || 'Bien immobilier')}` +
          `&total=${encodeURIComponent(totalAmount)}`;

        console.log('Redirecting to:', paymentUrl);
        setTimeout(() => { modal.style.display = 'none'; window.location.href = paymentUrl; }, 1500);
...
function setupPurchaseModal(property) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        console.log('Purchase created:', JSON.stringify(data));

        const purchaseId = data.data?.data?.[0]?.ID ||
                           data.data?.data?.ID       ||
                           data.data?.ID             ||
                           data.purchaseId;

        console.log('Purchase ID extracted:', purchaseId);

        const price   = parseFloat(property.Price1) || 0;
        const advance = Math.ceil(price * 0.10);

        showPurchaseMsg(msgDiv, 'success', '✓ Demande créée ! Redirection vers le paiement…');

        if (!purchaseId) {
          console.error('Could not extract purchase ID from:', JSON.stringify(data));
          showPurchaseMsg(msgDiv, 'error', '⚠️ Demande créée mais redirection impossible.');
          submitBtn.disabled = false;
          submitBtn.style.opacity = '1';
        ...
        const paymentUrl = `advance-payment.html?type=purchase` +
          `&id=${encodeURIComponent(purchaseId)}` +
          `&amount=${encodeURIComponent(advance)}` +
          `&property=${encodeURIComponent(property.title || 'Bien immobilier')}` +
          `&total=${encodeURIComponent(price)}`;
...
async function loadOwnerContact(property) {
  const ownerId = property?.User?.ID || property?.User?.id || property?.Added_User?.ID;
  if (!ownerId) return;

  try {
    const resp = await fetch(`/api/admin/users/detail/${ownerId}`, { credentials: 'include' });
    const data = await resp.json();
    if (!data.success || !data.user) return;

    ownerPhone = data.user.Phone_Number || data.user.phone_number || '';
    ownerEmail = data.user.Email        || data.user.email        || '';
    ownerName  = property.title         || data.user.display_value || 'ce bien';
  } catch (err) {
    console.error('loadOwnerContact error:', err);
  }
}

function setupContactPopup() {
  const triggerBtn = document.getElementById('contact-owner-btn');
  const overlay    = document.getElementById('contact-owner-overlay');
  if (!triggerBtn || !overlay) return;

  // Show or hide trigger depending on whether we have any contact info
  ...
  // Update popup subtitle with property name
  const subtitle = document.getElementById('contact-popup-subtitle');
  if (subtitle) subtitle.textContent = ownerName;

  // Show/hide individual option buttons
  const waBtn  = document.getElementById('contact-wa-btn');
  const emBtn  = document.getElementById('contact-em-btn');
  if (waBtn) waBtn.style.display = ownerPhone ? 'flex' : 'none';
  if (emBtn) emBtn.style.display = ownerEmail ? 'flex' : 'none';
  ...
  if (waBtn) {
    waBtn.addEventListener('click', () => {
      ...
      window.open(
        'https://wa.me/' + phone + '?text=' + encodeURIComponent('Bonjour, je vous contacte concernant le bien ' + ownerName),
        '_blank'
      );
  ...
  if (emBtn) {
    emBtn.addEventListener('click', () => {
      window.open(
        'https://mail.google.com/mail/?view=cm&to=' + encodeURIComponent(ownerEmail) +
        '&su=' + encodeURIComponent('Demande de contact - ' + ownerName),
        '_blank'
      );
      overlay.classList.remove('open');
    });
